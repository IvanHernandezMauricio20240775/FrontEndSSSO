...
async function ValidUser() {
    ...
    if (User) {
        ...
        const Employee = await getALlEmployeesAndUserByEmail(User.email);
        const DataEmployee = Employee.data


        const FirstName = document.getElementById("FirstNameUser");
        const SecondName = document.getElementById("SecondNameUser");
        const IMGUser = document.getElementById("UserImage");


        if (DataEmployee) {

            // No necesitas la línea 'FirstName.innerHTML = ''', el siguiente lo sobrescribe.
            FirstName.innerHTML = `${DataEmployee.first_name.toUpperCase()}`
            SecondName.innerHTML = `${DataEmployee.last_name.toUpperCase()}`
            IMGUser.src = `${DataEmployee.img_user}`

        } else {
            // Opcional: Manejar el caso de que el usuario no tenga datos de empleado
            console.error("No se encontraron datos de empleado para el usuario.");
